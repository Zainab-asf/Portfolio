  ...
  return (
    <footer style={{ borderTop: '2px solid var(--color-divider)', padding: '40px clamp(20px,5vw,64px)' }}>
      <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 15 }}>Codify</span>
        ...
        <span style={{ fontSize: 13, color: 'color-mix(in srgb,var(--color-text) 60%,transparent)' }}>© {new Date().getFullYear()} Codify</span>
      </div>
    </footer>
  );
}
